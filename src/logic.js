...

{
	...
	const AND = (...filters) => (word) => filters.every((filter) => filter(word));

	const ALL_WORDS = (word) => true;
	const WITHOUT_S = (word) => !/s/.test(word);
	const MAX_TWO_VOWELS = (word) => !/(?:.*?([aeiou])(?!.*\1)){3}/.test(word);
	const WITHOUT_S_MAX_TWO_VOWELS = AND(WITHOUT_S, MAX_TWO_VOWELS);

	const FILTER_OPTIONS = [
		[ALL_WORDS, 'all words'],
		[WITHOUT_S, 'without \'s\'', true],
		[MAX_TWO_VOWELS, 'max 2 vowels'],
		[WITHOUT_S_MAX_TWO_VOWELS, 'without \'s\' and max 2 vowels'],
	];
	...
	const BeeScoreCollection = class {
		...
		getBestBeeScores() {
			...
			let i = 1;
			while (i < this.scores.length && this.scores[i].points === best.points) {
				i++;
			}
			return this.scores.slice(0, i);
		}

		getWorstBeeScores() {
			const worst = this.scores[this.scores.length - 1];
			let i = this.scores.length - 2;
			while (i >= 0 && this.scores[i].points === worst.points) {
				i--;
			}
			return this.scores.slice(i + 1);
		}
...

	{
		...
		startButton.addEventListener('click', (event) => {
			...
			beeScores.compute(words, filter);
			const end = Date.now();
			const bestBeeScores = beeScores.getBestBeeScores();
			const worstBeeScores = beeScores.getWorstBeeScores();

			/* Exports */
			win.beeScores = beeScores;
			win.bestBeeScores = bestBeeScores;
			win.worstBeeScores = worstBeeScores;
			win.words = words;

			/* Output */
			if (beeScores.scores.length > 0) {
				appendLog('BEST\n');
				for (const best of bestBeeScores) {
					appendLog(best.toString(beeScores));
				}
				appendLog('\nWORST\n');
				for (const worst of worstBeeScores) {
					appendLog(worst.toString(beeScores));
				}
			} else {
				appendLog('no valid scores found');
			}

			console.log('best', bestBeeScores);
			console.log('worst', worstBeeScores);
